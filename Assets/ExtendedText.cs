...
public class ExtendedText : Text
{
    ...
    public void updateUnderline()
    {
        if (Underline && lineGameObject != null)
        {
			...
			lineImage.color = color;//canvasRenderer.GetColor();
            ...
            lineRectTransform.anchoredPosition = new Vector2(
                ...
                factor * (charactersInfo[underlineStart].cursorPos.y - (height * 1.2f) / 1.0f)
                );
            ...
            lineRectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, height / 8f);
        }
    }
